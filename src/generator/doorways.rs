...
use crate::components::{BoundingBox, Door, Position, Sprite};
use crate::map::*;
use crate::map_sprites::{FloorSprite, WallSpriteAlternate};

impl<'a> GameGenerator<'a> {
    pub(super) fn connect_rooms(&self, rng: &mut StdRng, map: &mut FloorMap, world: &mut World) {
        ...
        let mut doorways: Vec<_> = map
            .rooms()
            .flat_map(|(room_id, room)| {
                let grid = map.grid();

                // Find all edges that can be turned into a doorway on this room
                room.boundary().edge_positions().filter_map(move |edge| {
                    self.doorway_wall_adjacent_rooms(edge, room_id, grid)
                        .map(|pair| (edge, pair))
                })
            })
            .collect();
        ...
        while let Some(&(edge, pair)) = doorways.choose(rng) {
            ...
            doorways.retain(|&(_, (r1, r2))| {
                !connected_rooms.contains_key(&(r1, r2)) && !connected_rooms.contains_key(&(r2, r1))
            });
        }
        ...
        for ((room_id, _), edge) in connected_rooms {
            ...
            map.grid_mut()
                .get_mut(edge)
                .become_floor(room_id, FloorSprite::default());
    ...
    fn doorway_wall_adjacent_rooms(
        &self,
        edge: TilePos,
        room_id: RoomId,
        grid: &TileGrid,
