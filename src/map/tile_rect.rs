...
impl TileRect {
    ...
    pub fn tile_positions(self) -> impl Iterator<Item = TilePos> {
        (self.top_left.row..self.top_left.row + self.dim.rows).flat_map(move |row| {
            (self.top_left.col..self.top_left.col + self.dim.cols)
    ...
    pub fn edge_positions(self) -> impl Iterator<Item = TilePos> {
        ...
        (tl.col..tl.col + cols)
            .flat_map(
                ...
                move |col| {
                    once(TilePos { row: tl.row, col }).chain(once(TilePos { row: br.row, col }))
                }, // This code needs to avoid accidentally returning each corner twice
            )
            .chain((tl.row + 1..tl.row + rows - 1).flat_map(
                // Left and right edges
                move |row| {
                    once(TilePos { row, col: tl.col }).chain(once(TilePos { row, col: br.col }))
                },
            ))
    }
...
mod tests {
    ...
    fn rectangle_expand() {
        ...
        assert_eq!(
            rect.expand(2),
            TileRect::new(TilePos { row: 0, col: 0 }, GridSize { rows: 12, cols: 12 })
        );
        let rect = TileRect::new(TilePos { row: 1, col: 1 }, GridSize { rows: 10, cols: 10 });
        assert_eq!(
            rect.expand(2),
            TileRect::new(TilePos { row: 0, col: 0 }, GridSize { rows: 13, cols: 13 })
        );
        let rect = TileRect::new(TilePos { row: 2, col: 2 }, GridSize { rows: 10, cols: 10 });
        assert_eq!(
            rect.expand(2),
            TileRect::new(TilePos { row: 0, col: 0 }, GridSize { rows: 14, cols: 14 })
        );
        let rect = TileRect::new(TilePos { row: 2, col: 3 }, GridSize { rows: 10, cols: 12 });
        assert_eq!(
            rect.expand(2),
            TileRect::new(TilePos { row: 0, col: 1 }, GridSize { rows: 14, cols: 16 })
        );
    }
}
